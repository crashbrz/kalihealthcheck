...
printf '\e[91m\e[1m%s\n\e[0m' "Would you like to perform a deep checking ***EXPERIMENTAL and TAKES A LONG TIMEs*** [y/N]."
read -n 1 -r -s choice
if [[ "$choice" != *[yY]* ]] ;
then
	printf 'Deep checking skipped \e[1m'
else
...
do
	find "${line}" -iname "*" -type f | egrep -iv "(\.png|\.jpg|\.gif|\.jpeg|\.ico|\.svg|\.log|\.gz|\.tar|\.page|\.css|\.zip|exploitdb|password|shadow|android)" > "${filelist}" #retrives file list
...
do
	...
	userfilehash=$(cat ${userhashlist} | grep "${mergefilepath}$" | awk '{print $1}')
        #echo "==============================================================================" 
		#echo "Conteudo userfilehash     $userfilehash"
		#echo "Conteudo originalfilehash $originalfilehash"
        #echo "==============================================================================" 
	...
	if test "${userfilehash}" != "${originalfilehash}" ; then
		...
		dpkg -x *.deb .
		echo "Following  the curruption information:" 
		# diff -C0 "${mergefilepath}" "./${mergefilepath}" | egrep -i "(^\*\*\*.*.\*\*\*\*$|^\-\-\-.*.\-\-\-\-$)"  | egrep [0-9] | sed 's/\*\*\* /The corrupted line from your file is\: /g' | sed 's/\*\*\*\*//g' | sed 's/--- /The original line is: /g' | sed 's/ ----//g'
